...
export default function PanelView() {
  ...
  const filterByStatus = (data: AppData) => {
    ...
    // Calculate the overall status for the application card
    const overallStatus = getOverallStatus(data.mcStatus, data.appdStatus, data.guiStatus);
  
    // Filter by status
    const filteredByStatus = overallStatus === selectedStatus;
...
//   TabPanel,
// } from '@chakra-ui/react';
// import { MdDensityMedium, MdDensitySmall } from 'react-icons/md';
// import Banner from 'views/tech/panelView/components/Banner';
// import ApplicationCard from 'views/tech/panelView/components/ApplicationCard';
// import ServerCardList from './components/ServerCardList';
// import { SearchBar } from 'views/tech/panelView/components/Search';
...
//   // Function to handle tab change
//   const handleTabChange = (index: number) => {
//     setActiveTab(index === 0 ? 'application' : 'server');
//   };
...
//   useEffect(() => {
//     // Function to calculate overall status based on server statuses
//     const getOverallStatus = (mcStatus: string, appdStatus: string, guiStatus: string) => {
//       const statuses = [mcStatus, appdStatus, guiStatus];
//       if (statuses.includes('Red')) {
//         return 'Red';
...
//     // Update the statuses in the tableAppList based on the server statuses
//     tableAppList.forEach((app) => {
//       const appServers = tableNodeList.filter((node) => node.aaCode === app.aaCode);
//       const mcStatus = getOverallMCStatus(appServers);
//       const appdStatus = getOverallAppDStatus(appServers);
//       const guiStatus = getOverallGUIStatus(appServers);

//       app.mcStatus = mcStatus;
//       app.appdStatus = appdStatus;
//       app.guiStatus = guiStatus;
//     });
...
//     if (selectedAppAACode) {
//       // If there is a selected application, filter the server list based on its aaCode
//       const filteredServers = tableNodeList.filter((node) => node.aaCode === selectedAppAACode);
//       setFilteredServerList(filteredServers);
//     } else {
//       // If there is no selected application, show all server cards
//       setFilteredServerList(tableNodeList);
//     }
...
//         index={activeTab === 'application' ? 0 : 1}
...
//                 w='100%'
//                 transition='0.1s linear'
//                 bg={activeTab === 'application' ? 'navy.700' : 'transparent'}
//                 mt='15px'
//                 borderRadius='30px'
//               />
//             </Tab>
...
//                 w='100%'
//                 transition='0.1s linear'
//                 bg={activeTab === 'server' ? 'navy.700' : 'transparent'}
//                 mt='15px'
//                 borderRadius='30px'
//               />
//             </Tab>
...
//         </Flex>
//         <Text mt='25px' mb='36px' color={textColor} fontSize='2xl' ms='24px' fontWeight='700'>
//           {activeTab === 'application' ? filteredAppList.length : filteredServerList.length} Results
//         </Text>
//         <TabPanels>
//           <TabPanel px='0px'>
//             <SimpleGrid columns={{ base: 1, md: 2, xl: 4 }} gap='20px'>
...
//           <TabPanel px='0px'>
//             {/* Use the key prop to force a re-render of the ServerCardList component */}
//             {activeTab === 'server' && <ServerCardList key={tabKey} filteredServerList={filteredServerList} />}
//           </TabPanel>
//         </TabPanels>
//       </Tabs>
//     </Box>
